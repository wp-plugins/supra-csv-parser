...
class SupraCsvParser_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'supracsvparser'));
        }
        ...
        <div class="wrap">
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'supracsvparser'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'supracsvparser') ?>"/>
                </p>
            </form>

            <h2><?php _e('System Settings', 'supracsvparser'); ?></h2>
            <table class="form-table"><tbody>
            <tr><td><?php _e('System', 'supracsvparser'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'supracsvparser'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'supracsvparser');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'supracsvparser'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'supracsvparser');
                    }
                    echo '</span>';
                    ?>
                </td>
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'supracsvparser');
            case 'false':
                return __('false', 'supracsvparser');

            case 'Administrator':
                return __('Administrator', 'supracsvparser');
            case 'Editor':
                return __('Editor', 'supracsvparser');
            case 'Author':
                return __('Author', 'supracsvparser');
            case 'Contributor':
                return __('Contributor', 'supracsvparser');
            case 'Subscriber':
                return __('Subscriber', 'supracsvparser');
            case 'Anyone':
                return __('Anyone', 'supracsvparser');
        }
        return $optionValue;
    }
