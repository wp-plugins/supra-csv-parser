...
$(function() { 
    ...
    $('#delete_upload').live('click', function() {
        ...
        $.ajax({
          type: 'POST',
          data: {'action':'supra_csv','command':'delete_file','args':key},
          url: ajaxurl,
    ...
    $('#select_csv_file').live('change', function() {
        ...
        if(key) {
          ...
          $.ajax({
            type: 'POST',
            data: {'action':'supra_csv','command':'select_ingest_file','args':key},
            url: ajaxurl,
            success: function(msg){
                $('#supra_csv_ingestion_mapper').html(msg);
            }
          });
        }
    });
    ...
    $('#supra_csv_ingest_csv').live('click', function(e) {
        ...
        var data = $('#supra_csv_mapper_form').serialize();
        var filename = $('#supra_csv_mapper_form').data('filename');
